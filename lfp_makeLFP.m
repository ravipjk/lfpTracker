function lfp_makeLFP(expFolder)
    fprintf('\nData folder %s\n',expFolder);
    ...
    ncsFiles    = dir(fullfile(expFolder,'*.ncs'));
    ncsFiles    = sort({ncsFiles.name});
    nCh         = length(ncsFiles);

    cscData = cell(nCh,1);
    if ismac || isunix
        [ts,freq,Header] = Nlx2MatCSC_v3(fullfile(expFolder,ncsFiles{1}),[1 0 1 0 0],1,1,[]);
    else
        [ts,freq,Header] = Nlx2MatCSC(fullfile(expFolder,ncsFiles{1}), [1 0 1 0 0],1, 1,[]);
    end
    ...
    sampleFreq = 1250;
    % Round such that the period is a round number in us
    sampleFreq = 1e6/round(1e6/sampleFreq);
    ...
    for k=1:nCh
        ...
        fitObj = lfp_findPowerFit(cscData_60Notch);
        lfp.chData(k).fitObj = fitObj;
    end
    ...
    save(fullfile(x{1},'extractedLFP','subSampLFP.mat'),'lfp','-v7.3');
